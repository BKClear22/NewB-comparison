...
async function uploadImageToDrive(base64Image: string, fileName: string, accessToken: string, folderId: string) {
  try {
    ...
    if (!response.ok) {
      ...
      // Check if it's a disabled API error
      if (errorText.includes('accessNotConfigured') || errorText.includes('SERVICE_DISABLED')) {
        console.error('Google Drive API is not enabled. Enable it at: https://console.cloud.google.com/apis/api/drive.googleapis.com/overview');
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const row = [
      ...
      data.withdrawal || '', // F: Withdrawal
      data.previousTreatment || '', // G: Previous Treatment
      data.environment || '', // H: Environment
      Array.isArray(data.mentalHealth) ? data.mentalHealth.join(', ') : '', // I: Mental Health
      data.insuranceType || '', // J: Insurance Type
      data.insuranceProvider || '', // K: Insurance Provider
      insuranceCardLink || 'No image', // L: Insurance Card Image (link or "No image")
      data.insuranceReceivedHow || '', // M: How Insurance Received
      data.recoveryReadiness || '', // N: Recovery Readiness (0-10)
